...
*Setup instructions for a Gitian build of BitcoinOil Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinOil
...
- [Building BitcoinOil Core](#building-bitcoinoil-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoinoil-host/README.md).
...
Clone the git repositories for bitcoinoil and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinoil/bitcoinoil-core
```
...
in the bitcoinoil repository under 'Fetch and create inputs' to install sources which require
...
To build BitcoinOil Core (for Linux, OS X and Windows) just follow the steps under 'Setup and perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the bitcoinoil repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinoil/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitcoinoildev/bitcoinoil-core
...
For example:
```bash
URL=https://github.com/bitcoinoildev/bitcoinoil-core.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinoil-core=${COMMIT} --url bitcoinoil-core=${URL} ../bitcoinoil-core/contrib/gitian-descriptors/gitian-arm.yml
./bin/gbuild --commit bitcoinoil-core=${COMMIT} --url bitcoinoil-core=${URL} ../bitcoinoil-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinoil-core=${COMMIT} --url bitcoinoil-core=${URL} ../bitcoinoil-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinoil-core=${COMMIT} --url bitcoinoil-core=${URL} ../bitcoinoil-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcoinoil git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoinoil/contrib/gitian-descriptors/*|sort|uniq )
...
BTCOPATH=/some/root/path/bitcoinoil
SIGPATH=/some/root/path/bitcoinoil-detached-sigs

./bin/gbuild --url bitcoinoil=${BTCOPATH},signature=${SIGPATH} ../bitcoinoil/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinoil-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinoil-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinoil-osx-build.assert
```
...
[BITCOINOIL/bitcoinoil-sigs](https://github.com/BITCOINOIL/bitcoinoil-sigs) repository, or if that's not possible create a pull
request.
