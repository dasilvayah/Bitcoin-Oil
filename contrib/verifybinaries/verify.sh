...
###   This script attempts to download the signature file SHA256SUMS.asc from bitcoinoil.org
...
WORKINGDIR="/tmp/bitcoinoil"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bitcoinoil.org/bin/"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinoil-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinOil Core binary release signing key installed?"
   fi
