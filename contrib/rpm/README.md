...
The RPM spec file provided here is for BitcoinOil-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinoil.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinOil git repository](https://github.com/bitcoinoil/bitcoinoil) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinoil.conf` file.
...
in the bitcoinoil git repository. At present only an SVG version of the BitcoinOil
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinoil
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinOil git
master branch.
...
idea to build BitcoinOil Core against a static version of that library compiled
...
The `bitcoinoil.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinoil-server` package. At this point, most RPM based
...
The `bitcoinoil-server` package should have SELinux support. How to properly do
...
BitcoinOil-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinoil/). He has been
packaging BitcoinOil for Fedora at least since 2012.
...
4. I renamed the `bitcoinoil` package that contains the Qt GUI to `bitcoinoil-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinoil-xt` or `bitcoinoil-classic`. I wanted to make sure the general population
