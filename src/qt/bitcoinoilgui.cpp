...
#include "main.h"
#include "bitcoinoilgui.h"
#include "bitcoinoilunits.h"
...
const QString BitcoinOilGUI::DEFAULT_WALLET = "~Default";

BitcoinOilGUI::BitcoinOilGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void BitcoinOilGUI::createActions()
{
    ...
    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/bitcoinoil"), tr("&Overview"), this);
    ...
    openAction->setStatusTip(tr("Open a bitcoinoil: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitcoinOil command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinOilGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://s3.amazonaws.com/bitcoinoil-bootstrap/bootstrap-bitcoinoil_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the BitcoinOil Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void BitcoinOilGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(BitcoinOilUnits::Unit u, BitcoinOilUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(BitcoinOilUnits::name(u)), this);
...
void BitcoinOilGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void BitcoinOilGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool BitcoinOilGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void BitcoinOilGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool BitcoinOilGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BitcoinOilGUI::repairWallet()
{
...
void BitcoinOilGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BitcoinOilGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BitcoinOilGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BitcoinOilGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BitcoinOilGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}
...
void BitcoinOilGUI::gotoCommunityFundPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BitcoinOilGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BitcoinOilGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BitcoinOil network", "", count));
    if(walletFrame){
...
void BitcoinOilGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to BitcoinOil network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to BitcoinOil network..."));
...
void BitcoinOilGUI::showVotingDialog()
{
...
void BitcoinOilGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BitcoinOil"); // default title
...
#ifdef ENABLE_WALLET
void BitcoinOilGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void BitcoinOilGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(BitcoinOilGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
    ...
    menu(0)
{
    ...
    QList<BitcoinOilUnits::Unit> units = BitcoinOilUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const BitcoinOilUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(BitcoinOilUnits::name(unit)));
    }
...
{
    setText(BitcoinOilUnits::name(newUnits));
}
...
void BitcoinOilGUI::toggleStaking()
{
...
void BitcoinOilGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BitcoinOilGUI::updatePrice()
{
...
void BitcoinOilGUI::replyFinished(QNetworkReply *reply)
{
...
void BitcoinOilGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CBitcoinOilAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void BitcoinOilGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.bitcoinoil.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void BitcoinOilGUI::updateStakingStatus()
{
    updateWeight();
        ...
        else if (nLastCoinStakeSearchInterval && nWeight)
        {
            ...
            if ((fFoundPaymentRequest || fFoundProposal) && !this->fDontShowAgain && (this->lastDialogShown + (60*60*24)) < GetTimeNow()) {
                ...
                if (msgbox.clickedButton()==pButtonInfo) {
                    QString link = QString("https://bitcoinoil.org/en/community-fund/");
