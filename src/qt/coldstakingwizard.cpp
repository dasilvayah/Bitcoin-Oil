...
bool GetAddressesPage::validatePage()
{
    ...
    CBitcoinOilAddress stakingAddress(stakingAddressStr.toStdString());
    CBitcoinOilAddress spendingAddress(spendingAddressStr.toStdString());
...
void ColdStakingAddressPage::initializePage()
{
    ...
    coldStakingAddress = QString::fromStdString(CBitcoinOilAddress(stakingKeyID, spendingKeyID).ToString());

#ifdef USE_QRCODE
    QString uri = "bitcoinoil:" + coldStakingAddress;
    image->setText("");
    if(!uri.isEmpty())
    {
        // limit URI length
