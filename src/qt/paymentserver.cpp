...
const char* BIP71_MIMETYPE_PAYMENT = "application/bitcoinoil-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/bitcoinoil-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/bitcoinoil-paymentrequest";
const char* NIP01_MIMETYPE_SIGNEDMSG = "application/bitcoinoil-signedmsg";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the bitcoinoil: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(BITCOINOIL_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoinoil: URI
        {
            ...
            if (GUIUtil::parseBitcoinOilURI(arg, &r) && !r.address.isEmpty())
            {
                CBitcoinOilAddress address(r.address.toStdString());
    ...
    model(0)
{
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            LogPrintf("Cannot start bitcoinoil: click-to-pay handler\n");
        }
        else {
...
//
// OSX-specific way of handling bitcoinoil: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in bitcoinoil: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(BITCOINOIL_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoinoil: URI
    {
        ...
        if (uri.hasQueryItem("m") && uri.hasQueryItem("a")) // sign message URI
        {
            ...
            CBitcoinOilAddress addr(uri.queryItemValue("a").toStdString());
            if (!addr.IsValid())
            {
            ...
            QUrl fetchUrlConstructed(s.mid(BITCOINOIL_IPC_PREFIX.length()));
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CBitcoinOilAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom bitcoinoil addresses are not supported
        ...
        // BitcoinOil amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(BitcoinOilUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
