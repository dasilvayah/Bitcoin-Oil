    ...
    platformStyle(platformStyle)
{
    ...
    connect(ui->noBtcotechButton, SIGNAL(clicked()), this, SLOT(showBtcoTechDialog()));
...
void SendCoinsDialog::checkBtcotechServers()
{
    bool notEnoughServers = vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1;

    ui->noBtcotechLabel->setVisible(notEnoughServers);
...
void SendCoinsDialog::on_sendButton_clicked()
{
        ...
        if(entry->validate())
        {
            ...
            if(ui->anonsendCheckbox->checkState() != 0) {
                try
                {
                    ...
                    UniValue btcotechData = btcotech.CreateAnonTransaction(recipient.address.toStdString() , recipient.amount / (nTransactions * 2), nTransactions);
                    ...
                    if(serverNavAddresses.size() != nTransactions)
                    {
                        QMessageBox::warning(this, tr("Private payment"),
                                         "<qt>" +
                                         tr("BTCOTech server returned a different number of addresses.")+"</qt>");
                        valid = false;
                    }
                    ...
                    if(valid)
                    {
                      ...
                      for(unsigned int i = 0; i < serverNavAddresses.size(); i++)
                      {
                          ...
                          cRecipient.anondestination = QString::fromStdString(btcotech.EncryptAddress(recipient.address.toStdString(), pubKey.get_str(), nTransactions, i+(i==serverNavAddresses.size()?0:1), nId));
                          if(!find_value(btcotechData, "anonfee").isNull()){
                              cRecipient.anonfee = nAmountRound * ((float)find_value(btcotechData, "anonfee").get_real() / 100.0);
                              cRecipient.transaction_fee = find_value(btcotechData, "anonfee").get_real();
                          }else
                              valid = false;
    ...
    processSendCoinsReturn(prepareStatus,
        BitcoinOilUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    if (currentTransaction.fSpendsColdStaking && (!model->getOptionsModel()->getCoinControlFeatures() ||
        (model->getOptionsModel()->getCoinControlFeatures() && !CBitcoinOilAddress(CoinControlDialog::coinControl->destChange).IsColdStakingAddress(Params()))))
    {
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + BitcoinOilUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount);
        amount.append("</b>");
        ...
        if(txFee + anonfee > 0)
        {
            ...
            questionString.append(BitcoinOilUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee + anonfee));
            ...
            if(rcp.fSubtractFeeFromAmount && anonfee > 0)
            {
                ...
                questionString.append(BitcoinOilUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), anonfee));
            }

            if(rcp.isanon){
                questionString.append("<br>" + tr("Btcotech server fee:") +QString(" ")+ QString::number(rcp.transaction_fee) + "% "+ tr(rcp.fSubtractFeeFromAmount ? "" : "(already included)") + "<br>");
                if(rcp.fSubtractFeeFromAmount)
                    questionString.append("<span style='color:#aa0000;'>" + BitcoinOilUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount * ((rcp.transaction_fee/100))) + "</span> " + tr("will be deducted as Btcotech fee.") + "<br>");
...
void SendCoinsDialog::showBtcoTechDialog()
{
    btcotechsetup* setupBtcoTech = new btcotechsetup();
    setupBtcoTech->setWindowIcon(QIcon(":icons/bitcoinoil"));
    setupBtcoTech->setStyleSheet(Skinize());
                                 ...
                                 const CAmount& coldStakingBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(BitcoinOilUnits::formatWithUnit(0, balance) + (model->getOptionsModel()->getDisplayUnit() != 0 ?( " (" + BitcoinOilUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance) + ")") : ""));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(BitcoinOilUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::minimizeFeeSection(bool fMinimize)
{
    ...
    ui->noBtcotechButton  ->setVisible(fMinimize);
    //ui->horizontalLayoutSmartFee->setContentsMargins(0, (fMinimize ? 0 : 6), 0, 0);

    if(fMinimize)
        checkBtcotechServers();
    else
        ui->noBtcotechLabel  ->setVisible(false);
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
  ...
  //  else {
  //      ui->labelFeeMinimized->setText(BitcoinOilUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            BitcoinOilUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(BitcoinOilUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CBitcoinOilAddress addr = CBitcoinOilAddress(text.toStdString());
