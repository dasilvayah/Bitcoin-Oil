...
bool WalletModel::validateAddress(const QString &address)
{
  ...
  CBitcoinOilAddress addressParsed(address_str);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered bitcoinoil address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CBitcoinOilAddress(rcp.isanon ? rcp.destaddress.toStdString() : rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
        ...
        Q_FOREACH(const CRecipient &rcp, transaction.vecSend)
        {
          ...
          Q_FOREACH(const CWalletTx newTx, transaction.vTransactions)
          {
            ...
            Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
            {
              ...
              else if (!rcp.message.isEmpty()) // Message from normal bitcoinoil:URI (bitcoinoil:123...?message=example)
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
    {
        ...
        if (!rcp.paymentRequest.IsInitialized())
        {
            ...
            CTxDestination dest = CBitcoinOilAddress(strAddress).Get();
...
{
    QString strAddress = QString::fromStdString(CBitcoinOilAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CBitcoinOilAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CBitcoinOilAddress(sAddress).Get();
