...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s bitcoinoil-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  bitcoinoil-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded bitcoinoil transaction") + "\n" +
              "  bitcoinoil-tx [options] -create [commands]   " + _("Create hex-encoded bitcoinoil transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const string& strInput)
{
    ...
    CBitcoinOilAddress addr(strAddr);
    if (!addr.IsValid())
