        ...
        if(pblocktree->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CFund::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    CFund::CProposal proposal;
                    if(CFund::FindProposal(prequest.proposalhash, proposal)) {
                        CBitcoinOilAddress addr(proposal.Address);
                        if (!addr.IsValid())
                            continue;
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
...
void BitcoinOilStaker(const CChainParams& chainparams)
{
    LogPrintf("BitcoinOilStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinoil-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in BitcoinOilStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running BitcoinOilStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BitcoinOilStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BitcoinOilStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("BitcoinOilStaker: ProcessNewBlock, block not accepted");
        }
        else
        {
