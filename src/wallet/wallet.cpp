...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    if(hasPayment){
        ...
        CBitcoinOilAddress address2(address1);
    ...
    if (GetArg("-stakingaddress", "") != "" && !txNew.vout[txNew.vout.size()-1].scriptPubKey.IsColdStaking()) {
        ...
        if (stakingAddress.read(GetArg("-stakingaddress", "")))
        {
            try {
                ...
                {
                    address = CBitcoinOilAddress(find_value(addressMap, "all").get_str());
                }
                // Or use specified address if present
                if(find_value(addressMap, CBitcoinOilAddress(key.GetPubKey().GetID()).ToString()).isStr())
                {
                    address = CBitcoinOilAddress(find_value(addressMap, CBitcoinOilAddress(key.GetPubKey().GetID()).ToString()).get_str());
                }
        ...
        {
            address = CBitcoinOilAddress(GetArg("-stakingaddress", ""));
        }
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CBitcoinOilAddress(CScriptID(redeemScript)).ToString();
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, std::string strDZeel)
{
    ...
    txNew.strDZeel = (wtxNew.strDZeel != "" ? wtxNew.strDZeel : strDZeel).length() > 0 ?
                ...
                btcotech.EncryptAddress(std::to_string(GetAdjustedTime() + (rand() % 1<<8)),sPubKey);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-bitcoinoil-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CBitcoinOilAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CBitcoinOilAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CBitcoinOilAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CBitcoinOilAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CBitcoinOilAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CBitcoinOilAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CBitcoinOilAddress(dest).ToString(), key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakingaddress", strprintf(_("Specify a customised bitcoinoil address to accumulate the staking rewards.")));
