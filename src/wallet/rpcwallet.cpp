...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"bitcoinoiladdress\"    (string) The new bitcoinoil address\n"
            "\nExamples:\n"
    ...
    return CBitcoinOilAddress(keyID).ToString();
}
...
UniValue getcoldstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "Arguments:\n"
            "1. \"stakingaddress\"  (string, required) The bitcoinoil staking address.\n"
            "2. \"spendingaddress\" (string, required) The bitcoinoil spending address.\n\n"
            "\nExamples:\n"
    ...
    CBitcoinOilAddress stakingAddress(params[0].get_str());
    CKeyID stakingKeyID;
    if (!stakingAddress.IsValid() || !stakingAddress.GetKeyID(stakingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Staking address is not a valid BitcoinOil address");

    CBitcoinOilAddress spendingAddress(params[1].get_str());
    CKeyID spendingKeyID;
    if (!spendingAddress.IsValid() || !spendingAddress.GetKeyID(spendingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Spending address is not a valid BitcoinOil address");

    spendingAddress.GetKeyID(spendingKeyID);

    return CBitcoinOilAddress(stakingKeyID, spendingKeyID).ToString();
}


CBitcoinOilAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CBitcoinOilAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current BitcoinOil address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"bitcoinoiladdress\"   (string) The account bitcoinoil address\n"
            "\nExamples:\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"bitcoinoiladdress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"  (string, required) The bitcoinoil address to be associated with an account.\n"
    ...
    CBitcoinOilAddress address(params[0].get_str());
    if (!address.IsValid())
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"bitcoinoiladdress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"  (string, required) The bitcoinoil address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"bitcoinoiladdress\"  (string) a bitcoinoil address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinOilAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CBitcoinOilAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"bitcoinoiladdress\" amount ( \"comment\" \"comment-to\" \"anon-destination\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"  (string, required) The bitcoinoil address to send to.\n"
            ...
            "5. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a BTCOtech transaction \n"
    ...
    CBitcoinOilAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinOil address");
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            "createproposal \"bitcoinoiladdress\" \"amount\" duration \"desc\" ( fee dump_raw )\n"
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "BTCO is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"     (string, required) The bitcoinoil address where coins would be sent if proposal is approved.\n"
    ...
    CBitcoinOilAddress destaddress(Address);
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 BTCO\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CBitcoinOilAddress address(proposal.Address);

    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid BitcoinOil address.");
    ...
    std::string Secret = sRandom + "I kindly ask to withdraw " +
            std::to_string(nReqAmount) + "BTCO from the proposal " +
...
UniValue donatefund(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "                             The fund will receive less bitcoinoils than you enter in the amount field.\n"
            "\nResult:\n"
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
...
UniValue anonsend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "anonsend \"bitcoinoiladdress\" amount ( \"comment\" \"comment-to\" )\n"
    ...
    int nEntropy = GetArg("anon_entropy",BTCOTECH_DEFAULT_ENTROPY);
    ...
    UniValue btcotechData = btcotech.CreateAnonTransaction(params[0].get_str(), nAmount / (nTransactions * 2), nTransactions);
    std::vector<UniValue> serverNavAddresses(find_value(btcotechData, "anonaddress").getValues());

    if(serverNavAddresses.size() != nTransactions)
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "BTCOTech server returned a different number of addresses.");

    for(unsigned int i = 0; i < serverNavAddresses.size(); i++)
    {
        CBitcoinOilAddress serverNavAddress(serverNavAddresses[i].get_str());
        if (!serverNavAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bitcoinoil address provided by BTCOTech server");
    }
    ...
    CAmount nMinAmount = find_value(btcotechData, "min_amount").get_int() * COIN;
    UniValue pubKey = find_value(btcotechData, "public_key");
    ...
    for(unsigned int i = 0; i < serverNavAddresses.size(); i++)
    {
        ...
        string encryptedAddress = btcotech.EncryptAddress(params[0].get_str(), pubKey.get_str(), serverNavAddresses.size(), i+(i==serverNavAddresses.size()?0:1), nId);
...
UniValue getanondestination(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getanondestination \"bitcoinoiladdress\"\n"
    ...
    UniValue btcotechData = btcotech.CreateAnonTransaction(params[0].get_str());

    CBitcoinOilAddress serverNavAddress(find_value(btcotechData, "anonaddress").get_str());
    if (!serverNavAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bitcoinoil address provided by BTCOTech server");
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"bitcoinoiladdress\",     (string) The bitcoinoil address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CBitcoinOilAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CBitcoinOilAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CBitcoinOilAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"bitcoinoiladdress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"  (string, required) The bitcoinoil address to use for the private key.\n"
    ...
    CBitcoinOilAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"bitcoinoiladdress\" ( minconf )\n"
            "\nReturns the total amount received by the given bitcoinoiladdress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"  (string, required) The bitcoinoil address for transactions.\n"
    ...
    // BitcoinOil address
    CBitcoinOilAddress address = CBitcoinOilAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinOil address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tobitcoinoiladdress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a bitcoinoil address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tobitcoinoiladdress\"  (string, required) The bitcoinoil address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CBitcoinOilAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinOil address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The bitcoinoil address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CBitcoinOilAddress> setAddress;
    vector<CRecipient> vecSend;
    ...
    {
        CBitcoinOilAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid BitcoinOil address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "2. \"keysobject\"   (string, required) A json array of bitcoinoil addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) bitcoinoil address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"bitcoinoiladdress\"  (string) A bitcoinoil address associated with the keys.\n"
    ...
    return CBitcoinOilAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return CBitcoinOilAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CBitcoinOilAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinOilAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CBitcoinOilAddress, tallyitem>::iterator it = mapTally.find(address);
    ...
    if (fAllAccounts || (account == strAccount))
    {
        ...
        entry.push_back(Pair("canStake", (::IsMine(*pwalletMain, r.destination) & ISMINE_STAKABLE ||
                                          (::IsMine(*pwalletMain, r.destination) & ISMINE_SPENDABLE &&
                                           !CBitcoinOilAddress(r.destination).IsColdStakingAddress(Params()))) ? true : false));
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"bitcoinoiladdress\",    (string) The bitcoinoil address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"bitcoinoiladdress\",    (string) The bitcoinoil address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"bitcoinoiladdress\",   (string) The bitcoinoil address involved in the transaction\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            + HelpExampleCli("signmessage", "\"bitcoinoiladdress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; BitcoinOil server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of bitcoinoil addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) bitcoinoil address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the bitcoinoil address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CBitcoinOilAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid BitcoinOil address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CBitcoinOilAddress(address).ToString()));
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The bitcoinoil address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CBitcoinOilAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid bitcoinoil address");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
  ...
  if ((fHelp || params.size() != 1))
      throw runtime_error(
          ...
          "\nResolves the given OpenAlias address to a BitcoinOil address.\n"
          "\nArguments:\n"
          "1. \"address\"    (string) The OpenAlias address.\n"
          "\nExamples:\n"
          "\nGet information about an OpenAlias address\n"
          + HelpExampleCli("resolveopenalias", "\"donate@bitcoinoil.org\"")
      );
