...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"bitcoinoilprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"bitcoinoilprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CBitcoinOilAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CBitcoinOilAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CBitcoinOilAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const UniValue& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CBitcoinOilAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"bitcoinoiladdress\"\n"
            "\nReveals the private key corresponding to 'bitcoinoiladdress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"bitcoinoiladdress\"   (string, required) The bitcoinoil address for the private key\n"
            "\nResult:\n"
    ...
    return CBitcoinOilSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by BitcoinOil %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CBitcoinOilAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CBitcoinOilSecret(key).ToString(), strTime);
