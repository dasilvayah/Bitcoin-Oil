...
UniValue Btcotech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    ...
    UniValue btcotechData;
    btcotechData.setObject();
    btcotechData.pushKV("anondestination", encryptedAddress);
    btcotechData.pushKV("public_key", pubKey);
    btcotechData.pushKV("anonaddress", addrArray);
    btcotechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    btcotechData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return btcotechData;
  } else {
    throw runtime_error("Unable to send BTCOTech transaction, please try again.");
  }

}

vector<anonServer> Btcotech::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one BTCOTech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid BTCOTech server to send private payments.");
      return returnServers;
  }
...
UniValue Btcotech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured BTCOTech nodes are available right now.");
  }
...
UniValue Btcotech::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string Btcotech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",BTCOTECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int Btcotech::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * Btcotech::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool Btcotech::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue Btcotech::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue btcotechData;
        btcotechData.setObject();
        btcotechData.pushKV("min_amount", find_value(data, "min_amount"));
        btcotechData.pushKV("max_amount", find_value(data, "max_amount"));
        btcotechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return btcotechData;
      }
